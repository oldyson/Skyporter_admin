...
@Component({
	selector: 'app-userfamilylist',
	templateUrl: './userfamilylist.component.html',
	styleUrls: ['./userfamilylist.component.scss']
})
export class UserfamilylistComponent implements OnInit {
	...
	public getData(pageNumber): void {
		if (!this.isloading) {
			...
			const param = {
				...
				parentName: this.keyword != null ? ( this.keyword.length < 3 ? '' : this.keyword ) : ''
					...
					userfamilies.forEach(($ii) => {
						...
						temp = {
							...
							actions: [
								{
									label: 'View Detail',
									url: 'admin/user/family/detail?id=' + $ii.id,
									type: ''
								},
							]
						};
